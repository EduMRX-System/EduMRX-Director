...
// ═════════════════════════════════════════════════════════════════
// PHONE INPUT KOMPONENTI (TUZATILGAN VA INTEGRATSIYA QILINGAN)
// ═════════════════════════════════════════════════════════════════
...
const PhoneInput = ({ value, onChange, error }: PhoneInputProps) => {
    const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Agar foydalanuvchi tasodifan boshiga 998 yozsa yoki nusxalasa, uni qirqib tashlaymiz
...
// ═════════════════════════════════════════════════════════════════
// GLOBAL YANDEX MAPS SCRIPT LOADING MEXANIZMI
// ═════════════════════════════════════════════════════════════════
...
export default function AddLearningCenterModal({ onClose }: AddModalProps) {
    ...
    }, [isMounted, updateMarker, theme]); // 4. MUHIM: Dependency arrayga "theme" qo'shildi!
    ...
    // Yaratish Mutation so'rovi
    const { mutate: createCenter, isPending } = useMutation({
        mutationFn: async () => {
            ...
            const res = await API.post("super-admin/centers/", payload, {
                headers: { "Content-Type": "multipart/form-data" },
            });
            return res.data;
        },
        onSuccess: () => {
            toast.success(`${t("centers.add")} ${t("api_messages.success_created").toLowerCase()}`);
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > 2 * 1024 * 1024) {
            toast.error(t("centers.image_hint"));
            return;
        }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
                ...
                <h3 className="text-slate-900 dark:text-slate-100 text-[18px] font-semibold mb-4">
                    {t("centers.add_title") || "O'quv markazi qo'shish"}
                </h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="Masalan: Innovatsion Ta'lim"
                                className={inputCls}
                                required
                            />
                        </div>
                    ...
                    <div>
                        ...
                        <div className="flex gap-4 items-center border border-slate-200 dark:border-slate-700 rounded-xl p-3 bg-slate-50/50 dark:bg-slate-800/50">
                            <div className="relative w-16 h-16 min-w-16 rounded-lg border border-dashed border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 flex items-center justify-center overflow-hidden group">
                                {logoPreview ? (
                                    <>
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => { setSelectedFile(null); setLogoPreview(""); }}
                            ...
                            <div className="w-full">
                                ...
                                <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">{t("centers.image_hint") || "Maksimal hajm: 2MB"}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800 mt-6">
                        ...
                        >
                            ...
                            {isPending ? t("centers.creating") : t("centers.create_btn") || "Yaratish"}
                        </button>
                    </div>
                </form>
            </div>
